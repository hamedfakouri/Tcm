...
export class EducationCourseComponent implements OnInit {
  ...
  getList() {
    this.educationCourseService.getAll().subscribe(
  ...
  getItem(id: number) {
    this.educationCourseService.get(id).subscribe(
      (item: Educationcourse) => {
  ...
  deleteItem() {
    ...
    this.educationCourseService.delete(this.educationCourseItem).subscribe(
      () => {
        this.getList();
        this.clearForm();
